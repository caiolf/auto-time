...
{
  local
  VarSetCapacity(Rect, 16), DllCall("GetWindowRect"
  , "Ptr",DllCall("GetDesktopWindow", "Ptr"), "Ptr",&Rect)
  , zx:=NumGet(Rect, 0,"Int"), zy:=NumGet(Rect, 4,"Int")
  , zw:=NumGet(Rect, 8,"Int")-zx, zh:=NumGet(Rect, 12,"Int")-zy
  ...
  , ByRef zx:="", ByRef zy:="", ByRef zw:="", ByRef zh:="")
{
  ...
  if (zw<1 or zh<1)
    this.xywh2xywh(x,y,w,h,x,y,w,h,zx,zy,zw,zh)
  if (zw>bits.zw or zh>bits.zh or !bits.hBM)
  {
    if (bits.hBM)
...
{
  local
  return !(bits:=this.GetBitsFromScreen(0,0,0,0,0,zx,zy,zw,zh))
    or (x<zx or x>zx+zw-1 or y<zy or y>zy+zh-1 or !bits.Scan0)
    ? "0xFFFFFF" : Format("0x{:06X}",NumGet(bits.Scan0
    +(y-zy)*bits.Stride+(x-zx)*4,"uint")&0xFFFFFF)
}
  ...
  , ScreenShot:=1, ByRef rx:="", ByRef ry:="")
{
  ...
  Loop, %h%
  {
    j:=y+A_Index-1
    Loop, %w%
      i:=x+A_Index-1, c:=this.GetColor(i,j)
