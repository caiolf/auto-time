...
{   
    ...
    ; Rotina para o usario entrar somente de manha no tempo
    if (QuerManha)
        OnlyMorning(ASSTEC, ETAPA, PESSOA, MyDateTime, QuerManha, QuerTarde, QuerUmDia, Ignore, Ignore)
        ;Rotina para o usuario entrar somente de tarde no sistema   
    if (QuerTarde) 
     ;Rotina de um dia
...
Return
;Verificando o tempo da pessoa no sige
CONSULTANDO: 
{
    Gui, 2: Submit, nohide
    Gui, 2: Default
...
Return
 ;Verficando a mão de obra de determinada ASSTEC
CONSULTANDO_SIGE:
{   
    Gui, 2: Submit, nohide
    Gui, 2: Default
