...
{   
    ...
    if (QuerEntrar) ;Rotina de entrada
    {
        ...
        EntrarRetro(BoxHora, MyDateTime, QuerManha, QuerTarde, QuerUmDia, Ignore, Ignore)
        if WinExist("Warning")
            Return
    }
    ...
    if (QuerManha); Rotina para o usario entrar somente de manha no tempo
        OnlyMorning(ASSTEC, ETAPA, PESSOA, MyDateTime, QuerManha, QuerTarde, QuerUmDia, Ignore, Ignore)   
    if (QuerTarde) ;Rotina para o usuario entrar somente de tarde no sistema
        OnlyAfter(ASSTEC, ETAPA, PESSOA, MyDateTime, QuerManha, QuerTarde, QuerUmDia, Ignore, Ignore)
    ...
    if (QuerMaisDia) ;Rotina de varios dias
    {
        ...
        if(data2 < data1) ;Tratamento de erros
        {
...
CONSULTANDO: ;Verificando o tempo da pessoa no sige
{
    Gui, 2: Submit, nohide
    Gui, 2: Default
    CoordMode, Mouse, Relative ;Adaptando coordenadas para cada janela específica
...
CONSULTANDO_SIGE: ;Verficando a mão de obra de determinada ASSTEC
{   
    Gui, 2: Submit, nohide
    Gui, 2: Default
